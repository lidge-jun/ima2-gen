...
| `260824_novelai_provider_lane/` | NovelAI (NAI) image provider lane. wp0 docs (`000`-`003`) and wp1-wp2 landed on `dev`: registry, key plumbing, adapter, ZIP reader. `004` records a live V5 probe that corrected two roadmap assumptions. wp3 (server routing), wp4 (UI, doctor, i18n) and wp5 (verification and SoT sync) then landed, closing the lane; `evidence/` holds the live key round-trip, a real V5 generation, and the built-UI render grounding |

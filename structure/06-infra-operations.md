...
`scripts/release.sh` creates the version commit on a clean, fast-forwarded local `main`, verifies it, promotes that exact SHA to `preview`, and waits for registry/provenance proof. Only then does it create the stable tag and atomically push `main`, `dev`, and the tag; stable prepare/publish additionally require `preview` to remain on the same SHA. The OIDC workflow itself creates the GitHub Release after `latest` proof and attaches the release manifest and SBOM from the original provenance run. Local scripts never run `npm publish` and require the exact release Node/npm pair plus authenticated `gh`. If publishing succeeded but local recovery is still needed, `./scripts/release.sh finalize X.Y.Z` verifies immutable npm/remote-tag/provenance state, reuses `release-contract.mjs ensure-github-release`, and reconciles branches without rewinding any descendant work.

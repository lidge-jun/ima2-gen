...
function coerceStyleSheet(raw) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw))
        return null;
    const arr = (v, max = 6) => Array.isArray(v)
        ? v
    ...
    const hasContent = sheet.palette.length > 0 ||
...
export function renderStyleSheetPrefix(sheet) {
    ...
    const parts = [];
    if (sheet.medium)
        parts.push(`Medium: ${sheet.medium}.`);
    if (sheet.palette?.length)
        parts.push(`Palette: ${sheet.palette.join(", ")}.`);
    if (sheet.composition)
        parts.push(`Composition: ${sheet.composition}.`);
    if (sheet.mood)
        parts.push(`Mood: ${sheet.mood}.`);
    if (sheet.subject_details)
        parts.push(`Subject: ${sheet.subject_details}.`);
    if (sheet.negative?.length)
        parts.push(`Avoid: ${sheet.negative.join(", ")}.`);
    return parts.join(" ");
}

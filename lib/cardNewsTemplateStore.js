...
function normalizeLegacySlotKind(kind) {
    if (kind === "title")
        return { kind: "text", textKind: "headline" };
    if (kind === "body")
        return { kind: "text", textKind: "body" };
    if (kind === "cta")
        return { kind: "text", textKind: "cta" };
    if (kind === "image")
        return { kind: "image", textKind: null };
    if (kind === "text" || kind === "mixed" || kind === "safe-area")
        return { kind, textKind: null };
...
function validateTemplateAuthoring(template) {
    ...
    if (typeof template.name !== "string" || !template.name.trim())
        problems.push("name");
    if (typeof template.size !== "string" || !OUTPUT_SIZE_RE.test(template.size))
        problems.push("size");
    if (typeof template.stylePrompt !== "string" || !template.stylePrompt.trim())
        problems.push("stylePrompt");
    if (!Array.isArray(template.slots) || template.slots.length === 0)
        problems.push("slots");
    const ids = new Set();
    for (const slot of normalizeSlots(template.slots)) {
        if (ids.has(slot.id))
            problems.push(`duplicate slot ${slot.id}`);
        ids.add(slot.id);
        if (!PLACEMENTS.has(slot.placement))
            problems.push(`slot ${slot.id} placement`);
        if ((slot.kind === "text" || slot.textKind) && !slot.maxChars)
            problems.push(`slot ${slot.id} maxChars`);
    }
    if (template.recommendedOutputSizes &&

...
export async function extractStyleSheet(openai, { prompt, referenceDataUrl }) {
  if (!openai) {
    const err: any = new Error("No OpenAI client configured for style-sheet extraction");
  ...
  if (!raw) {
    const err: any = new Error("Empty response from style-sheet model");
  ...
  try {
    ...
    const err: any = new Error("Style-sheet model returned non-JSON");
  ...
  if (!sheet) {
    const err: any = new Error("Style-sheet shape invalid");

...
function diagnosticReasonFrom(err) {
    if (typeof err?.diagnosticReason === "string" && err.diagnosticReason)
...
export function errorCodeFrom(err) {
    ...
    if (typeof err.code === "string" && err.code)
        return err.code;
    if (err.cause)
        return errorCodeFrom(err.cause);
    return "UNKNOWN";
}
...
export function statusForErrorCode(code, fallback = 500) {
    if (code === "OAUTH_UNAVAILABLE" || code === "NETWORK_FAILED")
        return 503;
    if (code === "AUTH_CHATGPT_EXPIRED" || code === "AUTH_API_KEY_INVALID")
        return 401;
...
export function normalizeGenerationFailure(lastErr, options = {}) {
    ...
    if (typeof lastErr?.eventCount === "number") {
        const meta = [];
        if (lastErr.size)
            meta.push(`size=${lastErr.size}`);
        if (lastErr.quality)
            meta.push(`quality=${lastErr.quality}`);
        if (lastErr.model)
            meta.push(`model=${lastErr.model}`);

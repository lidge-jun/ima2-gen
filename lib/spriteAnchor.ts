...
export async function approveSpriteAnchor(recipeId: string, candidateAssetId: string): Promise<ApprovedSpriteAnchor> {
  const recipe = await spriteRecipeStore.approveAnchor(recipeId, candidateAssetId); const asset = getAsset(recipe.anchorAssetId!); if (!asset) throw spriteStoreError(404, "ANCHOR_ASSET_NOT_FOUND", "anchor asset not found"); return { asset, assetId: asset.id };
}
export async function requireApprovedSpriteAnchor(recipeId: string): Promise<ApprovedSpriteAnchor> {
  const recipe = await spriteRecipeStore.get(recipeId); if (!recipe) throw spriteStoreError(404, "SPRITE_RECIPE_NOT_FOUND", "sprite recipe not found"); if (recipe.anchorStatus !== "approved" || !recipe.anchorAssetId) throw spriteStoreError(400, "ANCHOR_NOT_APPROVED", "an approved idle anchor is required"); const asset = getAsset(recipe.anchorAssetId); if (!asset || asset.kind !== "image" || !asset.filePath) throw spriteStoreError(400, "ANCHOR_NOT_APPROVED", "approved anchor is no longer available"); return { asset, assetId: asset.id };
}

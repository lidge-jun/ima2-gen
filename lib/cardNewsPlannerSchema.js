...
function fallbackCopy(input = {}, kind = "body") {
    ...
    if (kind === "headline")
        return topic;
    if (lang === "ko")
        return brief || `${topic} 핵심 내용을 정리합니다.`;
    if (lang === "en")
        return brief || `Summarize the key point for ${topic}.`;
    return brief || topic;
}
...
function normalizeCard(card, role, index, input) {
    ...
    return {
        ...
        visualPrompt: stripExactVisibleText(asText(card?.visualPrompt, `${asText(role.promptHint, role.role)}, ${topic}`), textFields),
        textFields,
...
function validateTextField(field, path, errors) {
    ...
    if (typeof field.id !== "string")
        errors.push(`${path}.id must be string`);
    if (!CARD_NEWS_TEXT_KINDS.includes(field.kind))
        errors.push(`${path}.kind invalid`);
    if (typeof field.text !== "string")
        errors.push(`${path}.text must be string`);
    if (typeof field.text === "string" && !field.text.trim())
        errors.push(`${path}.text must not be empty`);
    if (!CARD_NEWS_RENDER_MODES.includes(field.renderMode))
        errors.push(`${path}.renderMode invalid`);
    if (!CARD_NEWS_PLACEMENTS.includes(field.placement))
        errors.push(`${path}.placement invalid`);
    if (!(typeof field.slotId === "string" || field.slotId === null))
        errors.push(`${path}.slotId invalid`);
    if (!CARD_NEWS_HIERARCHIES.includes(field.hierarchy))
        errors.push(`${path}.hierarchy invalid`);
    if (!(Number.isInteger(field.maxChars) || field.maxChars === null))
        errors.push(`${path}.maxChars invalid`);
    if (!(typeof field.language === "string" || field.language === null))
        errors.push(`${path}.language invalid`);
    if (!CARD_NEWS_TEXT_SOURCES.includes(field.source))
...
export function validatePlannerOutput(output, roleTemplate) {
    ...
    cards.forEach((card, index) => {
        ...
        if (card.order !== index + 1)
            errors.push(`card ${index + 1} order mismatch`);
        if (expected && card.role !== expected.role)
            errors.push(`card ${index + 1} role mismatch`);
        for (const key of ["headline", "body", "visualPrompt"]) {
            if (typeof card[key] !== "string")
                errors.push(`card ${index + 1} ${key} must be string`);
        }
        if (!Array.isArray(card.textFields))
            errors.push(`card ${index + 1} textFields must be array`);
        if (Array.isArray(card.textFields)) {
            card.textFields.forEach((field, fieldIndex) => validateTextField(field, `card ${index + 1} textFields ${fieldIndex + 1}`, errors));
            for (const field of card.textFields) {
                if (field?.renderMode === "in-image" &&
        ...
        if (!Array.isArray(card.references))
            errors.push(`card ${index + 1} references must be array`);
        if (card.locked !== false)
            errors.push(`card ${index + 1} locked must be false`);
    });
    if (errors.length)
        return { ok: false, repaired: false, errors };
    return {
        ok: true,
        repaired: false,
        errors: [],

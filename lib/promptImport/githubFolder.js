...
function normalizeItem(source, item) {
    ...
    if (type !== "file")
        return { warning: `${path || name}: folder-deferred` };
    if (!extension)
        return { warning: `${path || name}: unsupported-extension` };
    if (!insideFolder(source.path, path))
        return { warning: `${path || name}: outside-folder` };

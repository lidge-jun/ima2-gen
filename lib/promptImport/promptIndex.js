...
async function ensureSearchCache(ctx) {
  ...
  const limits = limitsFromCtx(ctx);
  const sources = getDefaultSearchSources();
  let changed = false;
  const warnings = [];
...
export async function searchCuratedPrompts(ctx, { q = "", sourceIds, limit } = {}) {
  ...
  const allowedIds = Array.isArray(sourceIds) && sourceIds.length
    ? new Set(sourceIds)
    : new Set(getDefaultSearchSources().map((source) => source.id));
...
export function getPromptImportSources() {
  return { sources: listCuratedSources() };
}

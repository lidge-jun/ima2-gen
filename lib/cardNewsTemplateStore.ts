...
function assertSafeId(id) {
  if (typeof id !== "string" || !/^[a-z0-9][a-z0-9-]{1,80}$/.test(id)) {
    const err: any = new Error("Invalid template id");
    err.status = 400;
...
function templateDir(ctx, templateId) {
  ...
  if (!normalizedDir.startsWith(normalizedRoot)) {
    const err: any = new Error("Template path escapes root");
    err.status = 400;
...
function validateTemplateAuthoring(template) {
  ...
  if (problems.length) {
    const err: any = new Error(`Template authoring metadata invalid: ${problems.join(", ")}`);
    err.status = 500;
...
async function readTemplate(ctx, templateId) {
  ...
  if (id !== templateId) {
    const err: any = new Error("Template id mismatch");
    err.status = 500;
  ...
  if (!entry) {
    const err: any = new Error("Template is not registered");
    err.status = 500;
...
export async function readTemplatePreview(ctx, templateId) {
  ...
  if (!existsSync(path)) {
    const err: any = new Error("Template preview not found");
    err.status = 404;
...
export async function readTemplateBaseB64(ctx, templateId) {
  ...
  const path = join(templateDir(ctx, templateId), filename);
  if (!existsSync(path)) {
    const err: any = new Error("Template base image not found");
    err.status = 404;

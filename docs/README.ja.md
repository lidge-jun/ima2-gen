...
`npx` で起動し、Codex OAuth でログインして、プロンプトを書きながら履歴、参照画像、スタイルシート、ノード分岐、multimode batch、Canvas Mode cleanup で反復できます。通常の画像生成には OpenAI API key は不要です。
...
- **Multimode batches**: 1つのプロンプトから複数候補を同時に走らせ、slot ごとの進行を見ながら良い結果から続けられます。
...
アプリの既定値は、速いローカル反復向けの **`gpt-5.4-mini`** です。安定したバランスを重視するなら **`gpt-5.4`** に切り替えることをおすすめします。
...
- zoom した画像で viewport pan と selection tool が分かれているため、annotation を誤って変えずに移動できます。
...
![file drop, GitHub folder preview, curated sources, discovered prompt results が見える prompt import dialog](../assets/screenshots/prompt-import-dialog.png)
...
| `ima2 ps` | 進行中ジョブを表示 |
| `ima2 cancel <requestId>` | 進行中 job を canceled としてマーク |

...
`npx` で起動し、Codex OAuth でログインして、プロンプトを書きながら履歴や参照画像、スタイルシート、ノード分岐、multimode batch、Canvas Mode cleanup を活用して試行錯誤（イテレーション）が可能です。通常の画像生成には OpenAI API key は不要です。

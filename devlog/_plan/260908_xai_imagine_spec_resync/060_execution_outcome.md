...
| GUI 동영상 입력 | **미완.** 드롭은 인식하고 CLI 경로를 안내하지만 GUI 진입점은 없다 |
| 드래그앤드랍 | 완료. 모드별 accept + 거부 사유 4개 언어 |

### 남은 작업의 규모

GUI 동영상 편집/연장은 소스 슬롯 컴포넌트, 편집/연장 모드 선택, 연장 길이 컨트롤,
`videoExtendStream` 연결, 결과 처리와 히스토리 반영이 필요하다. `PromptComposer`의
500줄 예산을 이미 소진했으므로 새 컴포넌트로 분리해야 한다. 별도 유닛이 맞다.
